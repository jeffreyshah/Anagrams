...
const Singleplayer: React.FC = () => {
  ...
  if (word.length >= 3 && word.length <= 6 && !validWords.has(word)) {
    ...
    foundWords.add(word);
 // Update the validWords state to trigger re-render
    } else {
  ...
  <div className="game-container">
    ...
    <div className="stats-container">
      ...
      <Image src="/images/lebron.webp" alt="Stats Icon" className="icon-image"  width={24} height={24}/>
      </div>
      ...
      {isGameOver && (
        ...
        <div className="word-list">
          ...
          <li key={index} style={{ listStyleType: "none" }}>{word}</li>
            ))}
          </div>
          </div>
          </div>)}
    </div>
    {!isGameOver && <div className="timer">Time Left: {timeLeft}s</div>}
    {isGameOver && (
